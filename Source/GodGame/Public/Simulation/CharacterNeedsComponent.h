...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FOnNeedChanged, const FGameplayTag&, NeedTag, float, NewValue, float, Delta);
...
protected:
	// Per-need satisfaction and decay state for the component owner; ::BaseNeeds and ::AdditionalNeeds are added onto this list at startup. 
	...
	static TArray<FGodGameNeedState> BaseNeeds;
...
public:
	...
	// Per-need satisfaction and decay state for the component owner; if none are specified, a default set of Needs and properties are used.
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Config|Needs", meta = (ClampMin="0.0", UIMin="0.0"))
	float DecayMultiplier = 1.0f;

	// Requested interval in seconds between need-decay updates.
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Config|Needs", meta = (ClampMin="0.05", UIMin="0.01"))
